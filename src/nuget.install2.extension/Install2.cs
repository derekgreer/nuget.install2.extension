...
namespace nuget.install2.extension
{
    ...
    public class Install2 : Command
    {
        ...
        public override void ExecuteCommand()
        {
            if(CommandVersion)
            {
                Console.WriteLine("{0} Version: {1}", GetType().Assembly.GetName().Name, GetType().Assembly.GetName().Version. ToString());
                return;
            }
        ...
        void RetrievePackage(Package package)
        {
            ...
            if (!string.IsNullOrEmpty(package.Version))
            {
                ...
                if (VersionUtility.TryParseVersionSpec(package.Version, out versionSpec))
                {
                    foreach (string source in Source)
                    {
                        IPackageRepository repository = _repositoryFactory.CreateRepository(source);
                        IPackage specificPackage = repository.FindPackage(package.Id, package.Version);

                        if (specificPackage != null)
                        {
                            command.Version = specificPackage.Version.ToString();
                            break;
                        }
                    }
                }
